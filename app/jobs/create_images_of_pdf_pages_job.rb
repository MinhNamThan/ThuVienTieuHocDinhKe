...
class CreateImagesOfPdfPagesJob < ApplicationJob
  ...
  def perform(book_id)
    ...
    doc_path = ActiveStorage::Blob.service.send(:path_for, book.doc_file.key)
    # set minimagick image wrapper for pdf stored in @doc.uplaod
    magick = MiniMagick::Image.open(doc_path)

    cable_broadcast("progress_before_conversion_channel", { total_pages: magick.pages.count, doc_id: book_id })
